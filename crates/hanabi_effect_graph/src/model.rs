...
/// UI layout for a graph: viewport transform, canvas positions and folding.
///
...
/// Scoped to the whole [`EffectGraph`] rather than per-emitter: node, stack,
/// source and emitter ids are already unique across every contained emitter,
/// so one flat layout covers any number of pipelines.
///
/// Every field but [`pan`] and [`zoom`] defaults, so an older file still
/// deserializes and a newer editor simply auto-places whatever it does not
/// find.
///
/// [`pan`]: GraphLayout::pan
/// [`zoom`]: GraphLayout::zoom
#[derive(Debug, Clone, PartialEq, Default, Serialize, Deserialize)]
pub struct GraphLayout {
    pub pan: (f64, f64),
    pub zoom: f64,
    pub node_pos: Vec<(NodeId, (f64, f64))>,
    /// Canvas position of each modifier stack, keyed by [`StackId`].
    ///
    /// Written only by editors that placed the three stacks of a pipeline
    /// independently. Current editors render one movable container per
    /// pipeline ([`pipeline_pos`]) and no longer write this, but still read it
    /// to derive a pipeline's initial position from an older file.
    ///
    /// [`pipeline_pos`]: GraphLayout::pipeline_pos
    #[serde(default)]
    pub stack_pos: Vec<(StackId, (f64, f64))>,
    /// Canvas position of each spawn source context (`CpuSpawner`/`GpuEvent`),
    /// keyed by [`SourceId`].
    ///
    /// Only sources that render as their own canvas item are stored; one hosted
    /// inside a pipeline's Emitter section is placed by that pipeline.
    #[serde(default)]
    pub source_pos: Vec<(SourceId, (f64, f64))>,
    /// Canvas position of each emitter pipeline container, keyed by
    /// [`EmitterId`].
    ///
    /// Empty for a file written before pipelines became one movable unit; the
    /// editor then derives each pipeline's position from [`stack_pos`].
    ///
    /// [`stack_pos`]: GraphLayout::stack_pos
    #[serde(default)]
    pub pipeline_pos: Vec<(EmitterId, (f64, f64))>,
    /// Pipeline sections the user folded to their header alone.
    ///
    /// A section is identified by whichever document id gives it stable
    /// identity — a [`StackId`] for a modifier phase, a [`SourceId`] for an
    /// Emitter section hosting a source, or an [`EmitterId`] for a sourceless
    /// one — so this is stored as the raw id those newtypes share. A section
    /// listed here renders folded; every other one renders expanded.
    #[serde(default)]
    pub collapsed_sections: Vec<NonZeroU32>,
}
    ...
    fn effect_graph_asset_round_trips() {
        ...
        let asset = EffectGraphAsset {
            ...
            layout: Some(GraphLayout {
                ...
                pipeline_pos: vec![(root_emitter, (80.0, 260.0))],
                collapsed_sections: vec![stack.0],
            }),
        };
